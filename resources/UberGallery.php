...
class UberGallery {
    ...
    protected $_now        = NULL;
	protected $_imageProps = NULL;
	protected $_databaseConf	= NULL;
    ...
    public function __construct() {
        ...
        if (file_exists($configPath)) {
			...
			$this->setImageProps($config['gallery_details']);
			$this->setDatabaseConf($config['database_settings']);
	...
	/*** DB Operation Starts here **********/
	
	public function connectDatabase(){
		$db_conf = $this->getDatabaseConf();
		$link = mysql_connect($db_conf['db_host'], $db_conf['db_user'], $db_conf['db_password']);
		if (!$link) {
		...
		if (!$db_selected) {
			return ('Can\'t use database : ' . mysql_error());
		}
	 
		return $link;
	}

	public function closeDatabase($link){
		return mysql_close($link);
	}
	
	public function postVote($imageid,$ipaddress,$imagename){
		$query = "INSERT INTO ratings (votewhen, image_id, ipaddress, image_name) VALUES (NOW(), '".$imageid."', '".$ipaddress."', '".mysql_real_escape_string($imagename)."')";
	...
	public function getVoteResults(){
		$query = "SELECT image_id,image_name, count(id) as totalvote FROM ratings group by image_id";
		...
		if(count($rows) > 0) {
			foreach ($rows as $key => $row) {
				$totalvote[$key]  = $row['totalvote'];
			}
			array_multisort($totalvote, SORT_DESC, $rows);
		}
     ...
     *
     * @param string $images list of image details
     * @access public
     */
     ...
     * @access public
     */
    public function setImageProps($images) {
        $this->_imageProps = $images;
    }
     ...
     * @access public
     */
    public function setDatabaseConf($conf) {
        $this->_databaseConf = $conf;
    }
                    ...
                    if ($file != "." && $file != "..") {
                        ...
                        if ($this->_isImage($realPath)) {
                            $dirArray[htmlentities(pathinfo($realPath, PATHINFO_BASENAME))] = array(
                                'real_path' => $realPath,
								'image_props'	=> $this->lookUpImageProps($file)								
                            );
                        }
                    }
                }
    ...
    private function _arraySort($array, $sortMethod, $reverse = false) {
        ...
        switch ($sortMethod) {
			...
			case 'custom':
				...
				for($sortIndex = 1; $sortIndex <= count($array); $sortIndex++ ) {
					foreach($array as $tempKey => $image_item) {
						if($image_item['image_props']['sort'] == $sortIndex){
							$sortedArray[$tempKey] = $image_item;
						}
					}
				}
	...
	public function lookUpImageProps($image_name){
		$all_image_props = $this->getImageProps();
		
		foreach ($all_image_props as $image_props) {
			if($image_props["file_name"] == trim($image_name)){
				return $image_props;
			}
		}
